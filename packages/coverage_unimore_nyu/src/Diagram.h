...
/*  Data structure diagramma di Voronoi --> DCEL: Doubly Connected Edge List
    ...
    Concettualmente ogni edge comune a due celle adiacenti � splittato in 2 half-edge, uno di
    propriet� di una cella , l'altro di propriet� della cella adiacente che condivide
...
//Prototipo classe (necessario perch� richiamata nella classe Diagram)
template<typename T>
...
//added    //Algoritmo di intersezione nel caso di Diagramma con solo sito/faccia centrale --> diagramma deve conicidere con box in modo che il sito stia fermo(densit� uniforme) o si muova verso p_t (densit� gaussiana)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        auto success = true;
        auto processedHalfEdges = std::unordered_set<HalfEdge*>();  //set che conterr� gli half-edge gi� processati (unordered_set solo per prestazioni)
        auto verticesToRemove = std::unordered_set<Vertex*>();  //set che conterr� i vertici da rimuovere (unordered_set solo per prestazioni)
        ...
        for (const auto& site : mSites)
        {
            ...
            auto inside = box.contains(halfEdge->origin->point); //variabile bool: true se il vertice di origine dell'half-edge � interno alla box, false se � esterno
            ...
            {   //Ciclo ripetuto fino all'esaurimento degli half-edge della faccia
                ...
                auto nextInside = box.contains(halfEdge->destination->point);   //variabile bool: true se il vertice di destinazione dell'half-edge � interno alla box, false se � esterno
                ...
                if (!inside && !nextInside)
                {
                    ...
                    {
                        verticesToRemove.emplace(halfEdge->origin); //inseriamo il vertice di origine dell'half-edge nel set dei vertici da rimuovere (solo il vertice di origine perch� tanto scansioniamo tutti gli half-edge)
                    ...
                    else if (nbIntersections == 2)
                    {
                        ...
                        //Controlliamo che nella lsta degli half-edge processati non ci sia gi� il suo twin
                        ...
                        {   //se lo �
                            ...
                            halfEdge->destination = halfEdge->twin->origin; //unordered_set rifiuta i doppioni quindi non verr� aggiunto di nuovo al set degli half-edge gi� scansionati
                        }
                        else
                        {   //se non � mai stato scansionato
                        ...
                        if (outgoingHalfEdge != nullptr)    //� ora di linkare il vettorino
                ...
                //Caso 3. half-edge � uscente dalla box --> nbIntersections=1
                else if (inside && !nextInside)
                {
                    //Numero intersezioni pu� essere anche > 1 oltre che = se consideriamo i corner
                    if (nbIntersections >= 1)
                    {   //se nella lista degli half-edge gi� processati � presente il gemello/twin dell'half-edge in esame
                        if (processedHalfEdges.find(halfEdge->twin) != processedHalfEdges.end())
                ...
                //Caso 4. half-edge � entrante nella box ---> nbIntersections=1
                else if (!inside && nextInside)
                {
                    //Numero intersezioni pu� essere anche > 1 oltre che = se consideriamo i corner
                    if (nbIntersections >= 1)
                    {
                        ...
                        if (outgoingHalfEdge != nullptr)    //se l'outgoingHalfEdge della cella � gi� stato trovato, possiamo linkare
                            link(box, outgoingHalfEdge, outgoingSide, halfEdge, intersections[0].side);
    ...
    std::vector<Vector2<T>> computeLloydRelaxation() const
    {
        ...
        }   //dopodich� si prosegue con la faccia successiva
        return centroids;   //restituisce vettore dei centroidi(che verr� sfruttato come vettore parametro per la funzione di costruzione del Diagram)
    }
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    // Triangulation
...
private:
    ...
    std::list<HalfEdge> mHalfEdges; //lista di struct HalfEdge (half-edges) --> ogni edge di una cella � splittato in 2 half-edge
    ...
    {   //Usiamo il reserve per evitare riallocazioni della memoria run time che potrebbero incidere negativamente sulle performance dell'algoritmo
        ...
        for (auto i = std::size_t(0); i < points.size(); ++i)   //pre-incremento = pi� prestazioni
    ...
    {   //Usiamo il reserve per evitare riallocazioni della memoria run time che potrebbero incidere negativamente sulle performance dell'algoritmo
        ...
        //Marchiamolo come sito index 0 cos� da averlo sempre in testa (e poterlo sfruttare in seguito per il calcolo del centroide)
        mSites.push_back(Diagram::Site{0, mainSite, nullptr});
        mFaces.push_back(Diagram::Face{&mSites.back(), nullptr});
        mSites.back().face = &mFaces.back();

        //Costruzione DCEL in base ad input forniti
        for (auto i = std::size_t(0); i < flt_points.size(); ++i)   //pre-incremento = pi� prestazioni
        {   //Inseriamo i siti input nel vettore dei siti --> costruzione mSites e mFaces
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Vertex* createVertex(Vector2<T> point)  //parametro: point(x,y)
    {
        ...
        mVertices.back().it = std::prev(mVertices.end());  //Sistemo l'iteratore sull'elemento/vertice appena introdotto-->prev(mVertices.end()) equivale a mVertices.end()-1, operazione necessaria perch� end punta sempre al nullptr
    ...
    HalfEdge* createHalfEdge(Face* face)   //parametro: puntatore ad una faccia incidente all'halfedge
    {
        ...
        mHalfEdges.back().it = std::prev(mHalfEdges.end()); //Sistemo l'iteratore sull'elemento/half-edge appena introdotto-->prev(mHalfEdges.end()) equivale a mHalfEdges.end()-1, operazione necessaria perch� end punta sempre al nullptr
        ...
        if (face->outerComponent == nullptr)
            face->outerComponent = &mHalfEdges.back();  //si evita cos� che ci possano essere facce che non puntano a nessun half-edge/outerComponent
    ...
    void link(Box<T> box, HalfEdge* start, typename Box<T>::Side startSide, HalfEdge* end, typename Box<T>::Side endSide)
    {
        ...
        auto side = static_cast<int>(startSide); //startSide � una variabile della enum class (LEFT,RIGHT,BOTTOM,UP) quindi faccio il cast trasformandola in numero intero
        while (side != static_cast<int>(endSide))   //finch� side non arriva al valore endSide
        {
            side = (side + 1) % 4;  //corrisponde a side++ ma quando arriva a 3 ristarta da 0 anzich� incrementare al valore 4. In questo modo c'� perfetta corrispondenza con la enum
            ...
            halfEdge = halfEdge->next;  //itero in avanti prendendo come half-edge il nuovo half-edge finch� non arrivo all'endside
        }
