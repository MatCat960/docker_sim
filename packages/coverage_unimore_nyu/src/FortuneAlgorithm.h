...
/*  Esecuzione dell'algoritmo Fortune per la generazione del Diagramma di Voronoi.
    L'algoritmo Fortune � diretto nel senso che non necessita della costruzione del
    grafo di Delaunay. La triangolazione di Delaunay pu� eventualmente essere calcolata
    in seguito.
    ...
    -Algoritmo Fortune restituisce di base un diagramma illimitato quindi � necessario un
     algoritmo di bounding per limitare il diagramma, clippando gli edge infiniti.
    -Algoritmo di Bounding sulla carta � sufficientemente indipendente dall'algoritmo di
     generazione del diagramma di Voronoi (quindi pu� essere adattato alle esigenze reali di percezione limitata?)
...
public:
    ...
    void construct()
    {
        ...
        for (auto i = std::size_t(0); i < mDiagram.getNbSites(); ++i)
            ...
            mEvents.push(make_unique<Event<T>>(mDiagram.getSite(i)));  //costruisce evento associandolo al Site
...
//        std::cout << "Lista ordinata degli eventi processati:" << std::endl;
        ...
        {
//            ////Stampa il nodo in posizione top della queue --> considerando che � in un ciclo while, viene stampata la lista degli eventi in ordine di elaborazione (debug)
...
//        mBeachline.print(std::cout);
    ...
    bool bound(Box<T> box)
    {
        ...
        //2. Clippare gli edge infiniti (fondamentale) --> step abbastanza semplice poich� si procede al calcolo delle intersezioni tra i lati del diagramma e i lati della BoundingBox
        ...
        if (!mBeachline.isEmpty())  //Se la beachline non � vuota (e non lo �)
        {   //Quindi a partire dal leftmost arc della beachline fino al rightmost, attraverso un ciclo while si procede al clippaggio di tutti gli edge infiniti residui con la BoundingBox (funzione boundedge)
            ...
            while (!mBeachline.isNil(arc->next))    //finch� non sono stati scansionati tutti gli archi della beachline
        ...
        //3. Chiudere le celle --> step che richiede pi� accortezza, suddivisibile in due sottostep principali
...
private:
    ...
    void handleSiteEvent(Event<T>* event)
    {
        ...
        //Left triplet (checkiamo se il precedente del leftArc � una foglia: se non lo � vuol dire che anche lui ha un growing edge quindi si deve calcolare il breakpoint che prima o poi si former� a sinista del middleArc)
        if (!mBeachline.isNil(leftArc->prev))
            addEvent(leftArc->prev, leftArc, middleArc); //aggiunta del CircleEvent alla queue
        //Right triplet (checkiamo se il successivo del rightArc � una foglia: se non lo � vuol dire che anche lui ha un growing edge quindi si deve calcolare il breakpoint che prima o poi si verificher� a destra del middleArc)
    ...
    {
        auto point = event->point;  //salvo il vertice/convergence point che si creer� con l'evento
        auto arc = event->arc;      //salvo l'arco associato all'evento che verr� shrinkatto con lo stesso
    ...
    //Rimozione dell'arco "shrinked into nothing" e pertanto non pi� necessario
    void removeArc(Arc<T>* arc, typename Diagram<T>::Vertex* vertex)
    {
        ...
        //Rimozione dell'arco "arc" che � "shrinked into nothing" e quindi non serve pi�
        ...
        //Concatenazione half-edges nuovi con quelli gi� esistenti
    ...
    {   //se la coordinata y del sito dell'arco left � minore della y del sito dell'arco right, il breakpoint si muove verso destra
    ...
    void setDestination(Arc<T>* left, Arc<T>* right, typename Diagram<T>::Vertex* vertex)
{
    ...
    {
        auto y = T();   //coordinata y della sweepline a cui si verificher� il CircleEvent
        ...
        //Controlliamo che la coordinata y a cui si verificher� sia minore dell'attuale posizione y della sweepline, se non lo � l'evento non � valido
        if (!almostLower(y, mSweepLineY))
            return;
        //Controlliamo che l'evento dia valido, checkando se il middleArc verr� shrinkato/distrutto dalla convergeneza dei due breakpoints
            ...
            (!rightBreakpointMovingRight && almostGreater(rightInitialX, convergencePoint.x))))
        {
            ...
            auto event = make_unique<Event<T>>(y, convergencePoint, middle);   //middle � l'arco che verr� shrinkato dall'evento
    ...
    Vector2<T> computeConvergencePoint(const Vector2<T>& point1, const Vector2<T>& point2, const Vector2<T>& point3, T& y) const
    {
        ...
        //Punti allineati, quindi non c'� soluzione
        if (almostZero(denom))
        ...
        y = center.y - r;   //coordinata y della sweepline alla quale si verificher� il CircleEvent esaminato
        return center;
    }
        ...
        VerticeOnFrontierContainer& vertices)
    {
        ...
        //Inizializzazione puntatori ai vertici (index � l'ID del sito dell'arco)
        if (vertices.find(leftArc->site->index) == vertices.end())  //cerchiamo gli array nell'unordered_map attraverso la key (.first), e come key prendiamo l'indice del sito del leftArc
            //Dualit� (unordered_)map-array, quindi si pu� sfruttare la notazione associative degli array: elem[key]
            vertices[leftArc->site->index].fill(nullptr);   //ovviamente se non � stata settato in precedenza non verr� trovato, quindi inizializziamolo
            //cos� abbiamo inizializzato a nullptr tutto l'array di puntatori corrispondenti alla key index interno al map
        if (vertices.find(rightArc->site->index) == vertices.end())
            vertices[rightArc->site->index].fill(nullptr);
        //Checkiamo che i vertici non siano gi� stati assegnati
    ...
    bool addCorners(const Box<T>& box, std::list<LinkedVertex>& linkedVertices, std::array<LinkedVertex*, 8>& cellVertices)
    {
        ...
        for (auto i = std::size_t(0); i < 5; ++i)
        {
            ...
            if (cellVertices[2 * side] == nullptr && cellVertices[2 * side + 1] != nullptr) //in altri termini se siamo nel LinkedVertex iniziale/sx di un side della box
            {
                ...
                success = cellVertices[2 * prevSide + 1] == nullptr && success; //se la cella precedente non ha un vertice proprio nel corner allora il suo vertice destro � nullptr poich� ultima cella del Side
    ...
    {   //Analizziamo tutti i side della box (partendo da quello left)
        for (auto side = std::size_t(0); side < 4; ++side)
        {
            ...
            {   //Linkiamo i vertici tramite half-edges
                auto halfEdge = mDiagram.createHalfEdge(mDiagram.getFace(i));   //i � la key del map ovvero l'ID del sito associato alla faccia
